/**
 * Smoke test runner — runs outside vitest, directly with tsx.
 * Usage: npx tsx test/smoke-runner.ts
 */
...
async function main() {
  if (!await ollamaAvailable()) {
    console.log("Ollama not available — aborting");
    process.exit(1);
  }
  ...
  for (const f of storyFiles) {
    console.log(`\n--- ${f} ---`);
    console.log(readFileSync(join(storiesDir, f), "utf-8").slice(0, 800));
  }

  // Assertions
  const errors: string[] = [];
  if (state.messages.length !== messages.length) errors.push(`messages: expected ${messages.length}, got ${state.messages.length}`);
  if (state.activeStories.length === 0) errors.push("activeStories: expected >0, got 0");
  if (storyFiles.length === 0) errors.push("storyFiles: expected >0, got 0");
